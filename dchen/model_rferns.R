...
#read in data
labels <- read.csv("../../train_labels.csv",na.string='',stringsAsFactors=F,row.names='id')
train <- read.csv("../../train_values.csv",na.string='',stringsAsFactors=F,row.names='id')
test <- read.csv("../../test_values.csv",na.string='',stringsAsFactors=F,row.names='id')
...
library(rFerns)
mat_dv <- sapply(labels, as.logical)
model <- rFerns(train_x, mat_dv)

P <- predict(model, test_x, scores=T)
Q <- P - min(P)
Q <- Q / max(Q)
